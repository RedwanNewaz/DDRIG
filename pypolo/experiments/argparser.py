...
def parse_arguments(verbose=True):
    ...
    # distributed robot settings
    parser.add_argument("--num-robots",
                        type=int,
                        default=5,
                        help="number of distributed robots")
    parser.add_argument("--alpha",
                        type=float,
                        default=0.85,
                        help="consensus parameter")

    parser.add_argument('--no-viz', action='store_true')
    parser.add_argument("--save-fig",
                        type=str,
                        default="",
                        help="provide output directory to save images")
    ...
    if len(args.save_fig):
        if not os.path.exists(args.save_fig):
            ...
            print("[+] new dir created @ ", args.save_fig)
